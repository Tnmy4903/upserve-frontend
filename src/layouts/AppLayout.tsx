...
export function AppLayout() {
  ...
  const labels: Record<string, string> = {
    ...
    "/app/record-directory": "Record directory",
    "/app/content": "Content management",
  };
  const section = Object.keys(labels).find(
    (path) =>
      location.pathname === path ||
      (path !== "/app" && location.pathname.startsWith(`${path}/`)),
  );
  useSEO({
    title: `${section ? labels[section] : "Workspace"} | Upserve`,
    description: "Private Upserve workspace.",
    path: location.pathname,
    indexable: false,
  });
  const breadcrumbParts = location.pathname.split("/").filter(Boolean);
  const breadcrumbLinks = breadcrumbParts
    .map((part, index, parts) => {
      ...
      return {
        label:
          labels[part] ||
          part.replace(/-/g, " ").replace(/^./, (value) => value.toUpperCase()),
  ...
  const mainLinks = [
    ...
    ...(user?.role === "client"
      ? [{ to: "/app/requirements", label: "Requirements", icon: FileText }]
      : []),
    ...(user?.role !== "sub_admin"
  ...
  const adminLinks = admin
    ? [
        { to: "/app/admin", label: "Admin overview", icon: ShieldCheck },
        ...
        {
          to: "/app/record-directory",
          label: "Record directory",
          icon: BriefcaseBusiness,
        },
        { to: "/app/content", label: "Content management", icon: Settings },
      ]
    : [];
  const mobileLinks = mainLinks.filter((link) =>
    [
      "/app",
      "/app/leads",
      "/app/requirements",
