...
function Cover({ src, alt, className = '' }: { src: string; alt: string; className?: string }) {
  ...
  if (!src || failed) return <div className={`content-cover content-cover--fallback ${className}`} role="img" aria-label={alt || 'Upserve content'}><span>Upserve</span></div>;
  return <img className={`content-cover ${className}`} src={src} alt={alt} loading="lazy" onError={() => setFailed(true)} />;
}
...
const finalServices = [
  ['Website Development', 'Custom, fast, SEO-ready websites and web applications.', 'We design and build websites and web applications that are fast, responsive, and built to rank.', 'Design, CMS setup, e-commerce (where needed), deployment.'],
  ['App Development', 'iOS and Android apps, native or cross-platform.', 'We build mobile apps for iOS and Android, native or cross-platform, designed for real engagement.', 'UI/UX design, development, API integration, app store submission.'],
  ['Software Development', 'Custom business tools built around how your team actually works.', 'We build custom software — CRMs, ERPs, dashboards, and internal systems — tailored to your workflow.', 'Requirement analysis, development, integration, training, support.'],
  ['AI & AI Agents', 'Chatbots and automation agents that save time and improve decisions.', 'We build AI tools — chatbots, automation agents, and applied AI features — that save time and support better decisions.', 'Custom model integration, analytics, continuous improvement.'],
  ['Automation & Internal Tools', 'Dashboards, workflows and integrations that remove manual work.', 'We build dashboards, internal tools, and automated workflows that eliminate repetitive manual work.', 'Process audit, tool build, APIs & integrations, cloud setup & deployment, team training.'],
  ['UI/UX & Brand Design', 'User-centered product design and a distinctive visual identity.', 'We design user-centered products and the visual identity that carries them — from research to a finished brand system.', 'Research, design systems, prototypes, logo direction, brand guidelines.'],
] as const;
...
export function ServicesPage() {
  ...
  return <main className="public-page content-page services-page">
    <div className="content-intro">
      ...
      <h1>What We Build</h1>
      <p>From websites to custom software and AI tools — here's what's included in each type of engagement.</p>
      <Link className="button-link" to="/contact">Tell Us What You're Building <ArrowRight size={16} aria-hidden="true" /></Link>
    </div>
    <section className="services-detail-grid" aria-label="Upserve services">
      {finalServices.map(([title, positioning, description, included], index) => <article className="services-detail-card" id={`service-${index + 1}`} key={title}>
        <span className="eyebrow">{String(index + 1).padStart(2, '0')}</span>
        <strong className="service-card-title">{title}</strong>
        <p className="service-card__focus">{positioning}</p>
        <p>{description}</p>
        <p className="service-card__included"><span>Includes</span>{included}</p>
      </article>)}
    </section>
    <section className="services-support" aria-labelledby="services-support-title">
      <div>
        <p className="eyebrow">A clear next step</p>
        <h2 id="services-support-title">Not Sure Which Service You Need?</h2>
        <p>Tell us what you're building — we'll help you figure out the right scope. Every engagement follows the same clear process, and every project runs through the platform we built to keep it organized.</p>
        <div className="services-support__links">
          <Link className="text-link" to="/#process-title">See how we work <ArrowRight size={16} aria-hidden="true" /></Link>
          <Link className="text-link" to="/showcase">See the platform <ArrowRight size={16} aria-hidden="true" /></Link>
        </div>
      </div>
    </section>
    <section className="home-cta" aria-labelledby="services-cta-title">
      <div>
        ...
        <p>Tell us what you're building — we'll help you figure out the right scope.</p>
      </div>
      <Link className="button-link" to="/contact">Tell Us What You're Building <ArrowRight size={16} aria-hidden="true" /></Link>
    </section>
  </main>;
...
export function ShowcasePage() {
  ...
  const capabilities = [
    ['Clarity', 'Keep the next decision and the current project context easy to see.'],
    ['Control', 'Give the team a shared workflow for requirements, scope, delivery, and payment.'],
  ...
  return <main className="public-page content-page showcase-page">
    <div className="content-intro">
      ...
      <p>This is the internal platform we use to manage every project — not a product for sale. It keeps requirements, delivery and communication in one place.</p>
    </div>
    <section className="showcase-frame" aria-labelledby="showcase-workflow-title">
      <div className="showcase-frame__bar"><span /><span /><span /><strong>Upserve workspace</strong></div>
      <div className="showcase-frame__body">
        <h2 id="showcase-workflow-title">From Requirement to Delivery, in One Place</h2>
        <p>One connected workflow keeps decisions, delivery and communication moving together.</p>
        <div className="showcase-workflow" aria-label="Requirement to delivery workflow">
          {workflow.map(([number, title, description]) => <div key={number}>
            <span>{number}</span>
            <strong>{title}</strong>
    ...
    <section className="showcase-annotations" aria-labelledby="showcase-capabilities-title">
      ...
      {capabilities.map(([title, description], index) => <article className="services-detail-card" key={title}>
        <span className="eyebrow">{String(index + 1).padStart(2, '0')}</span>
        <strong className="showcase-annotation-title">{title}</strong>
        <p>{description}</p>
      </article>)}
    </section>
    <section className="home-platform showcase-cta" aria-labelledby="showcase-cta-title">
      <div>
        <p className="eyebrow">Built for our own work</p>
        <h2 id="showcase-cta-title">Need a System Like This?</h2>
        <p>Tell us what your business needs to run more clearly — we'll start with the requirement.</p>
      </div>
      <Link className="button-link" to="/contact">Let's Talk <ArrowRight size={16} aria-hidden="true" /></Link>
    </section>
  </main>;
}
  ...
  return <main className="public-page content-page about-page">
    ...
    <section className="about-founder" aria-labelledby="founder-title">
      <div className="about-founder__media">
        <div className="about-founder__portrait"><img src="/founder_image.png" alt="Tanmay Jain, Founder and CEO of Upserve" loading="lazy" decoding="async" /></div>
        <div className="about-founder__identity">
          <div><strong>Tanmay Jain</strong><span>Founder &amp; CEO, Upserve · Software Entrepreneur · Digital Product Strategist</span></div>
          <a className="founder-linkedin" href="https://www.linkedin.com/in/tnmy4903/" target="_blank" rel="noreferrer"><span className="founder-linkedin__mark" aria-hidden="true">in</span><span>View LinkedIn profile</span><ExternalLink size={13} aria-hidden="true" /></a>
        </div>
      </div>
      <div className="about-founder__content">
        <p className="eyebrow">A note from the founder</p>
        <blockquote className="about-founder__quote">"Great products aren't built overnight — they're crafted with clarity, consistency, and a relentless focus on value."<cite>Tanmay Jain, Founder of Upserve</cite></blockquote>
        <h2 id="founder-title">Building products with clarity, consistency, and a relentless focus on value.</h2>
        <p>Tanmay Jain is the founder and CEO of Upserve, a software development agency based in Indore, India. He leads a team that builds custom software, mobile apps, AI-powered automation and cloud infrastructure — combining technical delivery with clear communication throughout.</p>
      </div>
    </section>
    <section className="about-video" aria-labelledby="about-video-title">
      <div className="about-video__heading"><h2 id="about-video-title">See How We Think About the Work</h2><p>A short introduction to how Upserve turns a clear requirement into a working product.</p></div>
      <div className="about-video__frame"><iframe src="https://www.youtube.com/embed/aqz-KE-bpKQ" title="Upserve company introduction" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div>
    </section>
    <section className="about-why" aria-labelledby="about-why-title"><div><h2 id="about-why-title">Why Upserve</h2></div><p>We've seen businesses struggle with agencies that overpromise, underdeliver, or disappear after launch. Upserve stays close to the product as it evolves — clear communication, and a real stake in the outcome.</p></section>
    <section className="about-expectations" aria-labelledby="about-expectations-title"><h2 id="about-expectations-title">What Clients Can Expect</h2><div className="about-expectations__grid">{['Clear communication', 'Defined scope', 'Structured requirements', 'Delivery-oriented execution'].map(item => <div key={item}><strong>{item}</strong></div>)}</div></section>
    <section className="home-cta" aria-labelledby="about-cta-title"><div><h2 id="about-cta-title">Tell Us What You're Building</h2></div><Link className="button-link" to="/contact">Start a Conversation <ArrowRight size={16} aria-hidden="true" /></Link></section>
  </main>;
}
...
export function ContactPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    const payload = { ...form, name: form.name.trim(), email: form.email.trim(), message: form.message.trim(), phone: form.phone.trim() || undefined, companyName: form.companyName.trim() || undefined, business: form.business.trim() || undefined };
    if (payload.name.length < 2 || payload.message.length < 10) { setState('Please enter your name and a message with at least 10 characters.'); requestAnimationFrame(() => document.querySelector<HTMLInputElement>('.public-form input')?.focus()); return; }
    setBusy(true);
    try {
      await contactApi.submit(payload);
      setForm({ name: '', email: '', phone: '', companyName: '', business: '', message: '' }); setState('success');
    } catch (error) { setState(error instanceof Error ? error.message : 'Unable to send your message.'); } finally { setBusy(false); }
  }
  return <main className="public-page contact-page"><div className="contact-layout"><div className="content-intro"><p className="eyebrow">Get in touch</p><h1>Ready to Start Your Project?</h1><p>Tell us what you're working on, what's getting in the way, and where you want to go.</p><div className="contact-details"><a href="mailto:tnmy4903@gmail.com"><span>Email</span><strong>tnmy4903@gmail.com</strong></a><a href="tel:+918858314903"><span>Phone</span><strong>+91 88583 14903</strong></a><div><span>Location</span><strong>Indore, Madhya Pradesh, India</strong></div></div><div className="contact-note"><strong>Prefer a quick call?</strong><span>Book a focused conversation about your goals and the right next step.</span><a className="text-link" href="tel:+918858314903">Book a Call <ArrowRight size={16} aria-hidden="true" /></a></div></div><form className="public-form" onSubmit={submit}><h2>Tell us about your project</h2><p className="muted">A little context helps us make our first conversation useful.</p>{state === 'success' ? <div className="success" role="status">Thanks for reaching out — we've received your message and will get back to you soon.</div> : state && <div className="error" role="alert">{state}</div>}<label>Your name<input required minLength={2} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></label><label>Work email<input required type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></label><label>Phone <span className="optional">Optional</span><input type="tel" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} /></label><label>Company name <span className="optional">Optional</span><input value={form.companyName} onChange={e => setForm({ ...form, companyName: e.target.value })} /></label><label>What does your business do? <span className="optional">Optional</span><input value={form.business} onChange={e => setForm({ ...form, business: e.target.value })} /></label><label>How can we help?<textarea required minLength={10} rows={6} value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} /></label><button disabled={busy}>{busy ? 'Sending…' : 'Send enquiry'}</button></form></div></main>;
}
